...
static int CPXPUBLIC MipIncumbentCallbackFunc(CPXCENVptr xenv, void *cbdata, int wherefrom, void *cbhandle, double objval, double *x, int *isfeas_p, int *useraction_p)
{
    ...
    printf("bestbound %f objval %f\n", bestobjval, objval);
    if (d->mipuppercutoffreached || bestobjval < d->mipuppercutoff / 1.1)*/
    if (d->mipuppercutoffreached)
    {
        *useraction_p = CPX_CALLBACK_FAIL;
    }
    else if (objval > d->mipuppercutoff)
    {
        printf("MipIncumbentCallbackFunc cutoff %f %f %d\n", d->mipuppercutoff, objval, *useraction_p);
        d->mipuppercutoffreached=true;
        *useraction_p = CPX_CALLBACK_DEFAULT;
    }
    return 0;
}

long GenModelCplex::SetObjUpperCutoff(double value)
{
    CplexData* d = (CplexData*)solverdata;

    d->mipuppercutoff = value;
    d->mipuppercutoffreached = false;
    CPXsetincumbentcallbackfunc(d->env, MipIncumbentCallbackFunc, d);
